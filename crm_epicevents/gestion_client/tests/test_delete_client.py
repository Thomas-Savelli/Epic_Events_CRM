...
def test_delete_client_success(mocker, custom_input):
    ...
    with patch('builtins.input', side_effect=custom_input):
        ...
        with patch('sys.stdout', new_callable=Mock) as mock_stdout:
            ...
            call_command('delete_client', str(client.id))

            # Vérification du message de succès
            assert f"Le Client avec l'ID {client.id} supprimé avec succès" in mock_stdout.write.call_args[0][0]


@pytest.mark.django_db
def test_delete_client_not_commercial_assigned(mocker, custom_input):
    # Créer un client fictif pour le test
    client = Client.objects.create(nom_complet='Client Test', email='test@example.com')
    ...
    with patch('builtins.input', side_effect=custom_input):
        ...
        with patch('sys.stderr', new_callable=Mock) as mock_stderr:
            ...
            try:
                ...
                assert str(ce) == "Accés refusé. Vous n'êtes pas le Commercial assigné à ce client."
...
def test_delete_client_permission_failure(mocker, custom_input):
    ...
    # Utilisation de patch pour simuler l'entrée utilisateur
    with patch('builtins.input', side_effect=custom_input):
        # Mock pour contourner la vérification de l'autorisation (rôle: 'support' différent de 'commercial')
